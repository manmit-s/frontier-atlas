        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=payload, headers=headers, timeout=aiohttp.ClientTimeout(total=25)) as resp:
                ...
                if resp.status == 429:
                    ...
                    raise LLMRateLimitError("DeepSeek 429 Rate Limit", retry_after=retry_after)

                if resp.status == 413:
                    raise LLMPayloadTooLargeError("DeepSeek 413 Payload Too Large")

                body = await resp.text()
                raise LLMProviderError(f"DeepSeek API error {resp.status}: {body[:200]}")
